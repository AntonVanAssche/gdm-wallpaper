...
if ! hash gresource 2>/dev/null; then
  echo "gresou   echo '      --uninstall              Remove changes and set original wallpaper (original gresource file)'
rce binary not found. "
  echo " "
...
if [ "$1" = "--help" ]; then
  ...
  echo '    Flags:'
  echo '      --uninstall							Remove changes and set original wallpaper (original gresource file)'
  echo '      --resize 0-1							Use built-in css template for image resize and alignment. Try this option for fix multi monitor issue. Use 0 for disable resize.'
  echo '        0 - background-repeat: repeat;'
  echo '        1 - background-position: 0 0;background-repeat: no-repeat;'
  exit 1
fi
...
if [ "$1" = "--resize" ]; then
  case "$2" in
    ...
    1) image_parameters="background-position: 0 0;background-repeat: no-repeat;";;
    *)
